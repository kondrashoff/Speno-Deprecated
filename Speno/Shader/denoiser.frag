...
vec3 denoise() {
    vec3 accum = safeLoad(accumulatedTexture, texCoords).rgb;
	vec3 normal = texture(normalTexture, texCoords).rgb;
    float depth = texture(positionTexture, texCoords).w;
    if(depth < 0.0) return accum;

	vec2 depth_gradient = vec2(dFdx(depth), dFdy(depth));
    float lum = luminance(accum);
	...
	for(int i = 0; i < filter_kernel.length(); i++) {
		...
		float temp_depth = texture(positionTexture, texCoords).w;
		if(temp_depth < 0.0) continue;

	    vec3 temp_color = safeLoad(accumulatedTexture, coord).rgb;
		float temp_variance = texture(varianceTexture, coord).r;
...
void main() {
    ...
    gbufferDenoised = safeLoad(accumulatedTexture, texCoords).rgb;
}